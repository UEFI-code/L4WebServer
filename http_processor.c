...
void reply_http_req(struct sockaddr_in *guestAddr, char *http_req, uint32_t req_len, char *http_res, uint32_t *res_len) {
    ...
    if (strncmp(http_req, "GET / ", 6) == 0) {
        char html_buf[512] = { 0 };
        uint32_t html_len = sprintf_s(html_buf, sizeof(html_buf), html_template, guestIP[0], guestIP[1], guestIP[2], guestIP[3], ntohs(guestAddr->sin_port));

        sprintf_s(http_res, TX_PayloadSize, http_header_template_200, html_len);
        strcat_s(http_res, TX_PayloadSize, html_buf);
